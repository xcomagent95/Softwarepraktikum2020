...
public class ToArray {
	...
	public ToArray(Loadzip dataset, String band) { //Uebergeben wird der Datensatz vom Typ Loadzip und das gewuenschte Band als String
		...
		datasetArray = new float[10000][10000]; //Array erzeugen 
	...
	/*public void writeArrayValues(Loadzip dataset, float[][] array, String band) { //Uebergeben werden: dataset vom Typ Loadzip, das zu fuellende Array sowie das gew�nschte Band
		Product product = null; //Product initialisieren
		try {
			product = ProductIO.readProduct(dataset.getFile()); //Product lesen
		} catch (IOException e) {
